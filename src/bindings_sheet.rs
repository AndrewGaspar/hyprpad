...
impl Sheet {
    ...
    pub fn set_layout(&mut self, layout: &str, c: &Config) {
        ...
        // The keyboard's own tab, likewise before the `sticks.enabled` check:
        // its D-pad row has nothing to do with the pointer either.
...
"guide+r1" = "workspace +1""#);
    ...
    fn a_padless_layouts_keyboard_tab_shows_the_highlight_not_the_pads() {
        ...
        // A controller with trackpads: two cursors and two pad clicks that
        // commit under them, exactly as before.
        let pads = sheet_from_toml(src);
        let rows = osk_rows(&pads);
        ...
        assert!(!rows.iter().any(|(chord, ..)| chord == "dpad"), "no nav rows with pads");
        ...
        assert_eq!(same(&osk_rows(&pads)), same(&rows));
